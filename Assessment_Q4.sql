SELECT
    ...
    TIMESTAMPDIFF(MONTH, u.date_joined, MAX(s.transaction_date)) AS tenure_months,  -- Tenure in months
    COUNT(s.id) AS total_transactions,  -- Total number of transactions
    ROUND(
        (COUNT(s.id) / NULLIF(TIMESTAMPDIFF(MONTH, u.date_joined, MAX(s.transaction_date)), 0)) * 12 
